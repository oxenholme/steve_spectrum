...
; MOVETHINGS and DRAWTHINGS. There are eight slots here, each one eight bytes
...
BEGIN:
  ...
  LD SP,$5C00             ; Set stack to printer buffer and drop through
                          ; to TITLESCREEN
...
STARTGAME:
  ...
  LD BC,$0100
  LDIR
; This entry point is used by the routines at ENDPAUSE (to teleport into a
; room), LOSELIFE (to reinitialise the room after Willy has lost a life),
; ROOMLEFT (when Willy has entered the room from the right), ROOMRIGHT (when
; Willy has entered the room from the left), ROOMABOVE (when Willy has entered
...
STARTGAME_1:
  ...
  DEC A                   ; Have we copied all eight entity definitions into
                          ; the entity buffer yet?
  JR NZ,STARTGAME_1       ; If not, jump back to copy the next one
...
ENDPAUSE_5:
  ...
  JP STARTGAME_0          ; Teleport into the room
...
; Now check whether any lives remain.
  ...
  LDIR
  JP STARTGAME_0          ; Reinitialise the room and resume the game
...
MOVEWILLY_3:
  LD A,(PIXEL_Y)          ; Pick up Willy's pixel y-coordinate from PIXEL_Y
  AND $0E                 ; Does Willy's sprite occupy six cells at the moment?
...
; Used by the routine at MAINLOOP.
MOVETHINGS:
  LD IX,ENTITYBUF         ; Point IX at the first byte of the entity buffer at
                          ; ENTITYBUF
; The entity-moving loop begins here.
MOVETHINGS_0:
  LD A,(IX+$00)           ; Pick up the first byte of the current entity
                          ; definition
  CP $FF                  ; Have we reached the end of the entity buffer?
  RET Z                   ; Return if so
  AND $03                 ; Keep only bits 0 and 1 (which determine the type of
                          ; entity)
  JP Z,MOVETHINGS_13      ; Jump to consider the next entity definition if this
                          ; one is an arrow or is empty
...
MOVETHINGS_4:
  ...
  AND $7F                 ; Reset bit 7
  CP (IX+$07)             ; Does A match the eighth byte of the entity
                          ; definition (54)?
  JP NZ,MOVETHINGS_13     ; If not, jump to consider the next entity definition
  LD A,(IX+$00)           ; Flip bit 7 of the first byte of the entity
  XOR $80                 ; definition: the rope has just changed direction and
  LD (IX+$00),A           ; will now swing back towards the centre
  JP MOVETHINGS_13        ; Jump to consider the next entity definition
...
; This guardian is moving right to left.
  ...
  SUB $20                 ; and 6 of the first byte of the entity definition)
  AND $7F
  LD (IX+$00),A
  CP $60                  ; Is it time to update the x-coordinate of the
                          ; guardian sprite?
  JR C,MOVETHINGS_13      ; If not, jump to consider the next entity definition
  ...
  JR MOVETHINGS_13        ; Jump to consider the next entity definition
MOVETHINGS_6:
...
MOVETHINGS_7:
  LD A,(IX+$00)           ; Update the guardian's animation frame (in bits 5
  ADD A,$20               ; and 6 of the first byte of the entity definition)
  OR $80
  LD (IX+$00),A
  CP $A0                  ; Is it time to update the x-coordinate of the
                          ; guardian sprite?
  JR NC,MOVETHINGS_13     ; If not, jump to consider the next entity definition
  LD A,(IX+$02)           ; Pick up the sprite's current screen x-coordinate
  AND $1F                 ; (0-31)
...
MOVETHINGS_9:
  LD A,(IX+$00)           ; Flip bit 3 of the first byte of the entity
  XOR $08                 ; definition (if bit 4 is set, the guardian's
  LD (IX+$00),A           ; animation frame is updated on every pass through
                          ; this routine; otherwise, it is updated on every
                          ; second pass when bit 3 is set)
...
MOVETHINGS_10:
  ...
  JR NC,MOVETHINGS_13     ; If the new y-coordinate is above the minimum value,
                          ; jump to consider the next entity definition
MOVETHINGS_11:
...
DRAWTHINGS_0:
  ...
  AND $07                 ; Keep only bits 0-2 (which determine the type of
                          ; entity)
  JP Z,DRAWTHINGS_22      ; Jump to consider the next entity definition if this
                          ; one is empty
...
; We are dealing with a horizontal or vertical guardian.
  ...
  AND $1F                 ; of the third byte of the entity definition
  ...
  INC HL
  LD (HL),C
  LD A,(IX+$03)           ; Pick up the fourth byte of the entity definition
...
DRAWTHINGS_1:
  ...
  JP DRAWTHINGS_22        ; Jump to consider the next entity definition
...
DRAWTHINGS_9:
  ...
  LD (IX+$03),A           ; definition; this holds the x-coordinate of the cell
                          ; in which the segment of rope under consideration
                          ; will be drawn
  LD (IX+$05),$80         ; Initialise the sixth byte of the entity definition
                          ; to 128 (bit 7 set); the value held here is used to
...
DRAWTHINGS_21:
  ...
  JR Z,DRAWTHINGS_22      ; jump to consider the next entity definition
...
;
; Used by the routines at DRAWLIVES and DRAWWILLY.
MANDAT:
...
; Used by the routine at STARTGAME.
;
; The following (empty) entity definition (0x00) is copied into the entity
; buffer at ENTITYBUF for any entity specification whose first byte is zero.
ENTITYDEFS:
  DEFB $00,$00,$00,$00,$00,$00,$00,$00
...
; are code remnants and unused data.
  ...
  DEFB $55,$00,$00,$00,$00,$00,$00,$00,$00 ; Floor
  DEFB $AA,$00,$00,$00,$00,$00,$00,$00,$00 ; Wall
  DEFB $FF,$00,$00,$00,$00,$00,$00,$00,$00 ; Nasty (unused)
